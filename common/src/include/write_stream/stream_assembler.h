...
#include "buffer/cuckoo_buffer.h"
#include "buffer/mem_pool.h"
#include "connection/cuckoo_io_client.h"
#include "log/logging.h"

#define CUCKOO_STORE_STREAM_MAX_SIZE (256 * 1024)
  ...
  public:
    ...
    bool Reserve(size_t reservedSize)
    {
        if (reservedSize > capacity) {
            ...
            if (tmpBuf == nullptr) {
                CUCKOO_LOG(LOG_ERROR) << "ExpandableMemory:: append() malloc failed";
                return false;
            }
  ...
  public:
    ...
    bool Append(const char *buf, size_t appendSize)
    {
        if (mem == nullptr) {
            ...
            if (mem == nullptr) {
                CUCKOO_LOG(LOG_ERROR) << "FixMemory get allocated nullptr";
                return false;
            }
        }
        if (size + appendSize > capacity) {
            CUCKOO_LOG(LOG_ERROR) << "FixMemory append data too large";
            return false;
        }
    ...
    char *mem = nullptr;
    size_t size = 0;
    size_t capacity = CUCKOO_STORE_STREAM_MAX_SIZE;
...
class WriteStream {
  public:
    ...
    int Push(CuckooWriteBuffer buf, off_t offset, uint64_t currentSize);
    ...
    void SetClient(std::shared_ptr<CuckooIOClient> cuckooIOClient);
    uint64_t GetSize();
  ...
  private:
    ...
    std::shared_ptr<CuckooIOClient> client = nullptr;
