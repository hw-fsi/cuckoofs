...
int OBSStorage::Init()
{
    if (!isInit.load()) {
        ...
        if (ret != 0) {
            CUCKOO_LOG(LOG_ERROR) << "obs initialize fail, ret is " << ret;
            return ret;
        }
        ret = set_online_request_max_count(REQUEST_MAX_COUNT);
        if (ret != 0) {
            CUCKOO_LOG(LOG_ERROR) << "obs set online request max count fail, ret is " << ret;
            return ret;
        }
        ...
        if (hostName.empty() || bucketName.empty() || accessKey.empty() || secretAccessKey.empty()) {
            CUCKOO_LOG(LOG_ERROR) << "Not enough info given to access obs";
            return -1;
        }
        if (HeadBucket() != 0) {
            CUCKOO_LOG(LOG_ERROR) << "Access obs bucket failed";
            return -1;
        }
        isInit.store(true);
        CUCKOO_LOG(LOG_INFO) << "successfully init obs, host name is " << hostName;
    }
    return 0;
}
...
int PutFileCallback(int bufferSize, char *buffer, void *callbackData)
{
    ...
    if (data->contentLength) {
        ...
        CuckooStats::GetInstance().stats[OBJ_PUT] += toRead;
    }
...
obs_status GetObjectDataCallback(int bufferSize, const char *buffer, void *callbackData)
{
    ...
    if (data->destBuffer != nullptr) {
        CuckooStats::GetInstance().stats[OBJ_GET] += bufferSize;
    ...
    if (data->fd != -1) {
        CuckooStats::GetInstance().stats[BLOCKCACHE_WRITE] += bufferSize;
...
ssize_t OBSStorage::ReadObject(const std::string &objectKey, uint64_t offset, uint64_t size, int fd, char *destBuffer)
{
    ...
    while (retryCount > 0) {
        ...
        if (OBS_STATUS_OK == data.retStatus) {
            ...
            CUCKOO_LOG(LOG_ERROR) << "ReadObject() " << objectKey << " failed: " << data.retStatus;
            CUCKOO_LOG(LOG_ERROR) << obs_get_status_name(data.retStatus);
            ret = -1;
        }
...
obs_status OBSStorage::ObsPutObject(const std::string &objectKey, const std::string &filePath, uint64_t contentLen)
{
    ...
    if (OBS_STATUS_OK != data.retStatus) {
        CUCKOO_LOG(LOG_ERROR) << "ObsPutObject " << objectKey << " error: " << data.retStatus;
        CUCKOO_LOG(LOG_ERROR) << obs_get_status_name(data.retStatus);
    }
...
ssize_t OBSStorage::PutBuffer(const std::string &objectKey, const char *buf, const uint64_t size, const uint64_t offset)
{
    ...
    if (OBS_STATUS_OK == data.retStatus) {
        ...
        CUCKOO_LOG(LOG_ERROR) << "Obs PutBuffer " << objectKey << " error: " << data.retStatus;
        CUCKOO_LOG(LOG_ERROR) << obs_get_status_name(data.retStatus);
        return -1;
    }
}
...
obs_status OBSStorage::ObsUploadFile(const std::string &objectKey, const std::string &filePath, uint64_t contentLen)
{
    ...
    CuckooStats::GetInstance().stats[OBJ_PUT] += contentLen;
    ...
    if (OBS_STATUS_OK != retStatus) {
        CUCKOO_LOG(LOG_ERROR) << "ObsUploadFile " << objectKey << " error: " << retStatus;
        CUCKOO_LOG(LOG_ERROR) << obs_get_status_name(retStatus);
    }
    return retStatus;
}
...
int OBSStorage::DeleteObject(const std::string &objectKey)
{
    ...
    if (OBS_STATUS_OK == attrBackData.retStatus) {
        CUCKOO_LOG(LOG_INFO) << "delete object " << objectKey << " successfully";
        obsRet = OBS_STATUS_OK;
    } else {
        CUCKOO_LOG(LOG_ERROR) << "delete object " << objectKey << " failed: " << attrBackData.error->message;
        obsRet = -1;
    }
    return obsRet;
}
...
int OBSStorage::GetStorageInfo(size_t &objNum, size_t &cap)
{
    ...
    if (ret_status == OBS_STATUS_OK) {
        ...
        CUCKOO_LOG(LOG_ERROR) << "GetStorageInfo() failed (" << obs_get_status_name(ret_status) << ").";
        return -1;
    }
}
...
int OBSStorage::GetQuota(uint64_t &quota)
{
    ...
    if (ret_status == OBS_STATUS_OK) {
        ...
        CUCKOO_LOG(LOG_ERROR) << "GetQuota() failed (" << obs_get_status_name(ret_status) << ").";
        return -1;
    }
}
...
int OBSStorage::CopyObject(const std::string &fromPath, const std::string &toPath)
{
    ...
    if (OBS_STATUS_OK != data.retStatus) {
        CUCKOO_LOG(LOG_ERROR) << "CopyObject " << fromPath << " to " << toPath
...
int OBSStorage::HeadBucket()
{
    ...
    if (ret_status != OBS_STATUS_OK) {
        CUCKOO_LOG(LOG_ERROR) << "HeadBucket " << bucketName << " failed: " << obs_get_status_name(ret_status);
        return -1;
    }
    return 0;
}
...
void OBSStorage::InitObsOptions(obs_options &option)
{
    ...
    if (isHttps) {
        ...
        CUCKOO_LOG(LOG_DEBUG) << "in https";
    } else {
        option.bucket_options.protocol = OBS_PROTOCOL_HTTP;
    }
    CUCKOO_LOG(LOG_DEBUG) << "InitObsOptions done";
    CUCKOO_LOG(LOG_DEBUG) << "host is " << hostName;
}