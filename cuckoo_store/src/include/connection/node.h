...
class StoreNode {
  private:
    ...
    int initStatus = 0;
    int nodeId;
    std::unordered_map<int, std::pair<std::string, std::shared_ptr<CuckooIOClient>>> nodeMap;
  ...
  public:
    ...
    CuckooIOClient *CreateIOConnection(const std::string &rpcEndPoint);
    void Delete();
    int GetNodeId();
    ...
    int GetBackupNodeId();
    std::shared_ptr<CuckooIOClient> GetRpcConnection(int nodeId);
    int GetInitStatus();
