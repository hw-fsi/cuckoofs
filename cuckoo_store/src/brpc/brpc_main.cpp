                                   ...
                                   google::protobuf::Closure *done)
{
    ...
    CUCKOO_LOG(LOG_INFO) << "Receive OpenFile rpc request, path = " << path << ", file size = " << size;

    std::shared_ptr<OpenInstance> openInstance = CuckooFd::GetInstance()->WaitGetNewOpenInstance(false);
    if (openInstance == nullptr) {
        response->set_error_code(-ENOMEM);
        response->set_physical_fd(0);
        CUCKOO_LOG(LOG_ERROR) << "OpenFile rpc request return with failure";
        return;
    }
    ...
    if (ret != 0) {
        ...
        uint64_t fd = CuckooFd::GetInstance()->AttachFd(path, openInstance);
        response->set_error_code(0);
        response->set_physical_fd(fd);
        openInstance->isOpened = true;
        CUCKOO_LOG(LOG_INFO) << "OpenFile rpc request return with cuckooFd = " << fd;
    }
}
                                    ...
                                    google::protobuf::Closure *done)
{
    ...
    CUCKOO_LOG(LOG_INFO) << "Receive CloseFile rpc request, fd = " << fd;

    std::shared_ptr<OpenInstance> openInstance = CuckooFd::GetInstance()->GetOpenInstanceByFd(fd);
    int ret = 0;
    if (!openInstance) {
        CUCKOO_LOG(LOG_ERROR) << "CloseFile(): fd " << fd << " not found";
    ...
    if (!buffer.empty()) {
        ...
        ret = CuckooStore::GetInstance()->WriteLocalFileForBrpc(openInstance.get(), buffer, offset);
        if (ret != 0) {
            CUCKOO_LOG(LOG_ERROR) << "WriteLocalFileForBrpc failed, ret = " << ret;
        }
    }

    int closeRet = CuckooStore::GetInstance()->CloseTmpFiles(openInstance.get(), flush, sync);
    ret = (ret == 0) ? closeRet : ret;

    if (ret != 0) {
        CUCKOO_LOG(LOG_ERROR) << "CloseFile rpc request failed, ret = " << ret;
    }
    if (!flush) {
        openInstance->isClosed = true;
        CuckooFd::GetInstance()->DeleteOpenInstance(fd, false);
    }
                                   ...
                                   google::protobuf::Closure *done)
{
    ...
    CUCKOO_LOG(LOG_INFO) << "Receive ReadFile rpc request, fd = " << fd << ", offset = " << offset
                         << ", size = " << readSize;
    ...
    std::shared_ptr<OpenInstance> openInstance = CuckooFd::GetInstance()->GetOpenInstanceByFd(fd);
    if (openInstance == nullptr) {
        CUCKOO_LOG(LOG_ERROR) << "ReadFile(): impossibly, fd " << fd << " not found";
        response->set_error_code(-EBADF);
        return;
    }
    ...
    if (buffer == nullptr) {
        CUCKOO_LOG(LOG_ERROR) << "Allocation failed for size " << allocSize;
        response->set_error_code(-ENOMEM);
        return;
    }

    int retSize = CuckooStore::GetInstance()->ReadFileLR(buffer, offset, openInstance.get(), allocSize);
    if (retSize < 0) {
        free(buffer);
        CUCKOO_LOG(LOG_ERROR) << "ReadFile rpc failed, fd = " << fd << ", error = " << retSize;
                                        ...
                                        google::protobuf::Closure *done)
{
    ...
    bool nodeFail = request->node_fail();
    CUCKOO_LOG(LOG_INFO) << "Receive ReadSmallFile rpc request, inode = " << inodeId << ", size = " << readSize;
    ...
    if (buffer == nullptr) {
        CUCKOO_LOG(LOG_ERROR) << "Allocation failed for size " << readSize;
        response->set_error_code(-ENOMEM);
        return;
    }

    int ret = CuckooStore::GetInstance()->ReadSmallFilesForBrpc(inodeId, path, buffer, readSize, oflags, nodeFail);
    if (ret < 0) {
        CUCKOO_LOG(LOG_ERROR) << "ReadSmallFile rpc failed, inodeId = " << inodeId << ", error = " << ret;
        response->set_error_code(ret);
        return;
    }
                                    ...
                                    google::protobuf::Closure *done)
{
    ...
    CUCKOO_LOG(LOG_INFO) << "Receive WriteFile rpc request, fd = " << fd;

    std::shared_ptr<OpenInstance> openInstance = CuckooFd::GetInstance()->GetOpenInstanceByFd(fd);
    if (openInstance == nullptr) {
        CUCKOO_LOG(LOG_ERROR) << "WriteFile(): impossibly, fd " << fd << " not found";
        response->set_error_code(-EBADF);
        return;
    }
    ...
    openInstance->writeCnt++;
    int ret = CuckooStore::GetInstance()->WriteLocalFileForBrpc(openInstance.get(), buffer, offset);
    if (ret < 0) {
        response->set_error_code(ret);
        return;
    }
                                     ...
                                     google::protobuf::Closure *done)
{
    ...
    const std::string &path = request->path();
    CUCKOO_LOG(LOG_INFO) << "Receive DeleteFile rpc request, inode = " << inodeId << " nodeid = " << nodeId;

    int ret = CuckooStore::GetInstance()->DeleteFiles(inodeId, -1, path);
    response->set_error_code(ret);
}
                                 ...
                                 google::protobuf::Closure *done)
{
    ...
    uint64_t ffiles = 0;
    uint64_t fffree = 0;
    int ret = CuckooStore::GetInstance()->StatFSForBrpc(path, fblocks, fbfree, fbavail, ffiles, fffree);
    if (ret < 0) {
        response->set_error_code(ret);
        return;
    }
                                               ...
                                               google::protobuf::Closure *done)
{
    ...
    CUCKOO_LOG(LOG_INFO) << "Receive TruncateOpenInstance rpc request, fd = " << fd << " size = " << size;

    std::shared_ptr<OpenInstance> openInstance = CuckooFd::GetInstance()->GetOpenInstanceByFd(fd);
    if (openInstance == nullptr) {
        CUCKOO_LOG(LOG_ERROR) << "TruncateOpenInstance(): impossibly, fd " << fd << " not found";
        response->set_error_code(-EBADF);
        return;
    }
    ...
    int ret = CuckooStore::GetInstance()->TruncateOpenInstance(openInstance.get(), size);
    response->set_error_code(ret);
}
                                       ...
                                       google::protobuf::Closure *done)
{
    ...
    uint64_t fd = request->physical_fd();
    off_t size = request->size();
    CUCKOO_LOG(LOG_INFO) << "Receive TruncateFile rpc request, fd = " << fd << " size = " << size;

    std::shared_ptr<OpenInstance> openInstance = CuckooFd::GetInstance()->GetOpenInstanceByFd(fd);
    if (openInstance == nullptr) {
        CUCKOO_LOG(LOG_ERROR) << "TruncateFile(): impossibly, fd " << fd << " not found";
        response->set_error_code(-EBADF);
        return;
    }
    ...
    openInstance->writeCnt++;
    int ret = CuckooStore::GetInstance()->TruncateFile(openInstance.get(), size);
    response->set_error_code(ret);
}
...
{
    cuckoo::brpc_io::RemoteIOServiceImpl remoteIOServiceImpl;
    if (server.AddService(&remoteIOServiceImpl, brpc::SERVER_DOESNT_OWN_SERVICE) != 0) {
        CUCKOO_LOG(LOG_ERROR) << "Fail to add service";
        return -1;
    }
    ...
    if (server.Start(point, &options) != 0) {
        CUCKOO_LOG(LOG_ERROR) << "Fail to start EchoServer";
        return -1;
    }
...

    {
        ...
        if (!isReady) {
            CUCKOO_LOG(LOG_INFO) << "init failed";
            return 0;
        }
    }
    // warm up
    ...
    CUCKOO_LOG(LOG_INFO) << "running successfully";
