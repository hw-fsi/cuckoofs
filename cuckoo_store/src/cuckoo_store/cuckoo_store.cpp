...
void CuckooStore::SetCuckooStoreParam(std::string &newNodeConfig) { nodeConfig = newNodeConfig; }

CuckooStore *CuckooStore::GetInstance()
{
    static CuckooStore instance;
    return &instance;
}

void CuckooStore::DeleteInstance()
{
...
int CuckooStore::GetInitStatus() { return initStatus; }

int CuckooStore::InitStore()
{
    int ret = GetInit().Init();
    if (ret != CUCKOO_SUCCESS) {
        CUCKOO_LOG(LOG_ERROR) << "Cuckoo init failed";
        return ret;
    }
    auto &config = GetInit().GetCuckooConfig();
    std::string rootPath = config->GetString(CuckooPropertyKey::CUCKOO_CACHE_ROOT);
    uint32_t totalDirectory = config->GetUint32(CuckooPropertyKey::CUCKOO_DIR_NUM);
    uint32_t blockSize = config->GetUint32(CuckooPropertyKey::CUCKOO_BLOCK_SIZE);
    CUCKOO_BLOCK_SIZE = blockSize;
    uint32_t bigFileReadSize = config->GetUint32(CuckooPropertyKey::CUCKOO_BIG_FILE_READ_SIZE);
    std::string clusterView = config->GetArray(CuckooPropertyKey::CUCKOO_CLUSTER_VIEW);
    asyncToObs = config->GetBool(CuckooPropertyKey::CUCKOO_ASYNC);
    persistToStorage = config->GetBool(CuckooPropertyKey::CUCKOO_PERSIST);
    uint32_t preBlockNum = config->GetUint32(CuckooPropertyKey::CUCKOO_PRE_BLOCKNUM);
    uint32_t threadNum = config->GetUint32(CuckooPropertyKey::CUCKOO_THREAD_NUM);
    float storageThreshold = GetStorageThreshold();
    parentPathLevel = GetParentPathLevel();
    bool ifStat = config->GetBool(CuckooPropertyKey::CUCKOO_STAT);
    isInference = config->GetBool(CuckooPropertyKey::CUCKOO_IS_INFERENCE);
    toLocal = config->GetBool(CuckooPropertyKey::CUCKOO_TO_LOCAL);
    std::string mountPath = config->GetString(CuckooPropertyKey::CUCKOO_MOUNT_PATH);

    CUCKOO_LOG(LOG_INFO) << "cuckoo_cache rootPath: " << rootPath;
    ...
    if (persistToStorage) {
        ...
        ret = storage->Init();
        if (ret != CUCKOO_SUCCESS) {
            CUCKOO_LOG(LOG_ERROR) << "storage init fail " << ret;
            return ret;
        }
    }
    ...
    if (ret != 0) {
        CUCKOO_LOG(LOG_ERROR) << "DiskCache start failed";
        return 1;
    }
    MemPool().GetInstance().init(CUCKOO_BLOCK_SIZE, preBlockNum);
    ...
    if (storeThreadPool == nullptr || storeThreadPool->Start() != 0) {
        CUCKOO_LOG(LOG_ERROR) << "Cuckoo threadpool init failed";
        return 1;
    }
#ifdef ZK_INIT
    ret = StoreNode::GetInstance()->SetNodeConfig(rootPath);
    if (ret != 0) {
        CUCKOO_LOG(LOG_ERROR) << "Cuckoo StoreNode init failed";
        return ret;
    }
#else
...
int CuckooStore::PathToNodeId(std::string &path)
{
...
void CuckooStore::AllocNodeId(OpenInstance *openInstance)
{
...
bool CuckooStore::ConnectionError(int err) { return err > 0; }

bool CuckooStore::IoError(int err) { return err < 0; }

/*---------------------- write ----------------------*/

// WriteLocalFileForBrpc can only be called from brpc server
int CuckooStore::WriteLocalFileForBrpc(OpenInstance *openInstance, butil::IOBuf &buf, off_t offset)
{
    ...
    if (!DiskCache::GetInstance().PreAllocSpace(sizeToAdd)) {
        CUCKOO_LOG(LOG_ERROR) << "WriteLocalFileForBrpc(): Can not pre-allocate enough space!";
        return -ENOSPC;
    }
    if (!isDirect) {
        while (writeSize > 0) {
            ...
            if (nwrite < 0 || nwrite > (ssize_t)writeSize) {
                ...
                if ((uint64_t)offset > currentSize) {
                    ...
                    if (!DiskCache::GetInstance().Update(openInstance->inodeId, offset)) {
                        CUCKOO_LOG(LOG_ERROR) << "WriteLocalFileForBrpc(): DiskCache Update failed!";
            ...
            CuckooStats::GetInstance().stats[BLOCKCACHE_WRITE] += nwrite;
            writeSize -= nwrite;
            offset += nwrite;
        }
    } else {
        ...
        if (bytes_cut < writeSize) {
            CUCKOO_LOG(LOG_ERROR) << "WriteLocalFileForBrpc(): cntn not enough data in IOBuf";
            DiskCache::GetInstance().FreePreAllocSpace(sizeToAdd);
            free(alignedBuf);
            return -EIO;
        }
        ...
        if (retSize < 0) {
            int err = errno;
            CUCKOO_LOG(LOG_ERROR) << "WriteLocalFileForBrpc(): pwrite failed" << strerror(err);
            DiskCache::GetInstance().FreePreAllocSpace(sizeToAdd);
            return -err;
        }
        CuckooStats::GetInstance().stats[BLOCKCACHE_WRITE] += retSize;
    }
...
int CuckooStore::WriteFile(OpenInstance *openInstance, const char *buf, size_t size, off_t offset)
{
    CUCKOO_LOG(LOG_INFO) << "WriteFile(): called";
    int ret = 0;
    CuckooWriteBuffer cuckooBuf{buf, size};
    ...
    if (openInstance->preReadStarted.load() && !openInstance->preReadStopped.exchange(true)) {
        CUCKOO_LOG(LOG_INFO) << "WriteFile(): StopPreReadThreaded";
    ...
    ret = openInstance->writeStream.Push(cuckooBuf, offset, openInstance->currentSize.load());
    if (ret != 0) {
        CUCKOO_LOG(LOG_ERROR) << "WriteFile(): openInstance->stream.push() failed";
        openInstance->writeFail = true;
        return ret;
    }
 ...
 * Called by fuse
 */
int CuckooStore::ReadFile(OpenInstance *openInstance, char *buf, size_t size, off_t offset)
{
    int ret = 0;
    int err = 0;
    CuckooReadBuffer cuckooBuf{buf, size};
    ...
    if (openInstance->writeStream.GetSize() > 0) {
        ...
        CUCKOO_LOG(LOG_INFO) << "In ReadFile(): Persisting the written";
        ret = openInstance->writeStream.Complete(openInstance->currentSize.load(), true, false);
        if (ret != 0) {
            CUCKOO_LOG(LOG_ERROR) << "In ReadFile(): persist written before read failed";
            return ret;
        }
    }
    ...
    if (openInstance->originalSize >= READ_BIGFILE_SIZE || (openInstance->oflags & O_ACCMODE) != O_RDONLY) {
        ...
        if (!openInstance->isOpened.load()) {
            ...
            ret = OpenFile(openInstance);
            if (ret != 0) {
                CUCKOO_LOG(LOG_ERROR) << "In ReadFile(): big file OpenFile() failed";
                return ret;
            }
            openInstance->isOpened = true;
        }
        ...
        int readSize = ReadToBuffer(cuckooBuf, openInstance, offset);
        if (readSize < 0) {
            CUCKOO_LOG(LOG_ERROR) << "In ReadFile(): ReadToBuffer() failed";
        }
        return readSize;
    } else {
        /* read small files */
        ...
        if (offset + size <= (size_t)openInstance->readBufferSize) {
            err = memcpy_s(cuckooBuf.ptr, cuckooBuf.size, openInstance->readBuffer.get() + offset, size);
            if (err != 0) {
 ...
 */
bool CuckooStore::StartPreReadThreaded(OpenInstance *openInstance)
{
    int fileBlocks = (openInstance->currentSize + CUCKOO_BLOCK_SIZE - 1) / CUCKOO_BLOCK_SIZE;

    if (!openInstance->readStream.Init(openInstance, fileBlocks, CUCKOO_BLOCK_SIZE)) {
        return false;
    }
 ...
 */
void CuckooStore::StopPreReadThreaded(OpenInstance *openInstance)
{
    /* stop only once */
 ...
 */
int CuckooStore::ReadToBuffer(CuckooReadBuffer buf, OpenInstance *openInstance, off_t offset)
{
    CUCKOO_LOG(LOG_INFO) << "CuckooStore::ReadToBuffer(): called";
 ...
 */
int CuckooStore::RandomRead(CuckooReadBuffer buf, OpenInstance *openInstance, off_t offset)
{
 ...
 */
int CuckooStore::SequenceRead(CuckooReadBuffer buf, OpenInstance *openInstance, off_t /*offset*/)
{
 ...
 */
ssize_t CuckooStore::ReadFileLR(char *readBuffer, off_t offset, OpenInstance *openInstance, size_t readBufferSize)
{
    ...
    if (StoreNode::GetInstance()->IsLocal(openInstance->nodeId)) {
        if (openInstance->physicalFd != UINT64_MAX && !fileLock.TestLocked(openInstance->inodeId, LockMode::X)) {
            ...
            CuckooStats::GetInstance().stats[BLOCKCACHE_READ] += checkReadLength;
            ...
            if (retSize != checkReadLength) {
                ...
                if (err == EAGAIN) {
                    ...
                    if (retSize != checkReadLength) {
                        err = errno;
                        CUCKOO_LOG(LOG_ERROR) << "In ReadFileLR(): pread fd = " << openInstance->physicalFd
                                              << " failed : " << strerror(err);
                        retSize = -err;
                    }
                } else {
                    CUCKOO_LOG(LOG_ERROR)
        ...
        if (!openInstance->remoteFailed) {
            ...
            if (retSize != checkReadLength) {
                CUCKOO_LOG(LOG_ERROR) << "In ReadFileLR(): read remote failed: " << strerror(-retSize) << ", for node "
                                      << openInstance->nodeId;
    ...
    if (retSize < 0 && !openInstance->isRemoteCall && persistToStorage) {
        CUCKOO_LOG(LOG_DEBUG) << "ReadFile from obs : " << openInstance->path;
        retSize = storage->ReadObject(openInstance->path.substr(1), offset, readBufferSize, -1, readBuffer);
        if (retSize < 0) {
            CUCKOO_LOG(LOG_ERROR) << "In ReadFileLR(): obs ReadObject() failed";
            retSize = -EIO;
        }
    }
    return retSize;
 ...
 * Called by fuse
 */
int CuckooStore::OpenFile(OpenInstance *openInstance)
{
    CUCKOO_LOG(LOG_INFO) << "OpenFile(): called by " << (openInstance->isRemoteCall ? "remote" : "fuse");
    int ret = 0;
    int err = 0;
    if (openInstance->physicalFd == UINT64_MAX) {
        ...
        if (!StoreNode::GetInstance()->IsLocal(openInstance->nodeId)) {
            ...
            if (ret != 0) {
                CUCKOO_LOG(LOG_ERROR) << "OpenFile(): open remote file failed";
                return ret;
            }
        } else {
            ...
            if (DiskCache::GetInstance().Find(openInstance->inodeId, true)) {
                ...
                if (localFd < 0) {
                    ...
                    CUCKOO_LOG(LOG_ERROR) << "OpenFile(): open local file " << fileName << " failed: " << strerror(err);
                    return -err;
                }
                openInstance->physicalFd = static_cast<uint64_t>(localFd);
                CUCKOO_LOG(LOG_INFO) << "OpenFile(): Opened existed local file " << fileName
                ...
                if ((openInstance->oflags & O_ACCMODE) != O_RDONLY) {
                    ...
                    if ((openInstance->oflags & O_CREAT) == 0 && openInstance->originalSize > 0) {
                        if (!persistToStorage) {
                            if (access(fileName.c_str(), F_OK) == 0) {
                                CUCKOO_LOG(LOG_ERROR) << "OpenFile(): cache file " << fileName
                                                      << " missing in diskCache but exists in ext for write";
                            } else {
                                CUCKOO_LOG(LOG_ERROR) << "OpenFile(): cache file " << fileName << " missing in write";
                            }
                            return -ENOENT;
                        }
                        CUCKOO_LOG(LOG_INFO) << "OpenFile(): Loading evicted write only cache file";
                    ...
                    CUCKOO_LOG(LOG_INFO) << "OpenFile(): create local cache file " << fileName
                                         << " , fd = " << openInstance->physicalFd;
                } else {
                    ...
                    if (!persistToStorage) {
                        if (access(fileName.c_str(), F_OK) == 0) {
                            ...
                            CUCKOO_LOG(LOG_ERROR) << "OpenFile(): cache file " << fileName << " missing in read";
                        }
                        return -ENOENT;
                    }
        ...
        ret =
            openInstance->writeStream.SetFd(openInstance->physicalFd); // set physicalFd in local, or cuckooFd in remote
    }
    return ret;
}
 ...
 */
int CuckooStore::DownLoadFromStorage(OpenInstance *openInstance, bool isSync, bool toBuffer)
{
    ...
    if (!lockerPtr->isLocked()) {
        CUCKOO_LOG(LOG_INFO) << "DownLoadFromStorage(): No need to load obs, other acquired the lock, abort";
        return 0;
    }

    /* if file exists in disk cache, pin directly if it is sync */
    if (DiskCache::GetInstance().Find(openInstance->inodeId, isSync)) {
        CUCKOO_LOG(LOG_INFO) << "DownLoadFromStorage(): No need to load obs, other created local file, abort";
        return 0;
    }

    if (!DiskCache::GetInstance().PreAllocSpace(fileSize)) {
        CUCKOO_LOG(LOG_ERROR) << "DownLoadFromStorage(): Can not pre-allocate enough space!";
        return -ENOSPC;
    }
    ...
    if (fd < 0) {
        int err = errno;
        CUCKOO_LOG(LOG_ERROR) << "DownLoadFromStorage(): Create local file for loading failed: " << strerror(err);
    ...
    auto loadObs = [=, this]() {
        ...
        close(fd);
        if (size < 0) {
            CUCKOO_LOG(LOG_ERROR) << "DownLoadFromStorage(): Loading file from obs failed";
            if (std::remove(fileName.c_str()) != 0) {
                CUCKOO_LOG(LOG_ERROR) << "DownLoadFromStorage(): Delete obs tmp file failed" << strerror(errno);
            }
            size = -EIO;
        } else {
 ...
 */
int CuckooStore::OpenFileFromRemote(OpenInstance *openInstance, bool largeFile)
{
    ssize_t ret = EHOSTUNREACH;
    int nodeCnt = StoreNode::GetInstance()->GetNumberofAllNodes();
    std::shared_ptr<CuckooIOClient> cuckooIOClient = nullptr;

    /* Loop on connection error, switch node after that */
    for (int i = 0; i < nodeCnt && ConnectionError(ret); i++) {
        cuckooIOClient = StoreNode::GetInstance()->GetRpcConnection(openInstance->nodeId);
        if (cuckooIOClient) {
            if (largeFile) {
                ret = cuckooIOClient->OpenFile(openInstance->inodeId,
                                               openInstance->oflags,
                                               openInstance->physicalFd,
                ...
                ret = cuckooIOClient->ReadSmallFile(openInstance->inodeId,
                                                    openInstance->originalSize,
                                                    openInstance->path,
        ...
        if (ConnectionError(ret)) {
            ...
            openInstance->nodeId = StoreNode::GetInstance()->AllocNode(openInstance->inodeId);
            CUCKOO_LOG(LOG_WARNING) << "OpenFileFromRemote(): failed at node  " << backupNode << " , switch "
                                    << openInstance->nodeId;
    ...
    if (ret == 0 || largeFile) {
        if (ConnectionError(ret)) {
            CUCKOO_LOG(LOG_ERROR) << "OpenFileFromRemote(): connection failed for all nodes";
        } else if (IoError(ret)) {
            CUCKOO_LOG(LOG_ERROR) << "OpenFileFromRemote(): open remote file " << openInstance->path
                                  << " failed: " << strerror(-ret) << ", for node " << openInstance->nodeId;
        } else {
            openInstance->writeStream.SetClient(cuckooIOClient);
        }
    ...
    if (persistToStorage) {
        CUCKOO_LOG(LOG_WARNING) << "OpenFileFromRemote(): small read remote failed, read obs instead";
        ret = storage->ReadObject(openInstance->path.substr(1),
                                  ...
                                  openInstance->readBuffer.get());
        if (ret < 0) {
            CUCKOO_LOG(LOG_ERROR) << "OpenFileFromRemote(): obs ReadObject() " << openInstance->path << " failed";
            return -EIO;
        }
        ret = 0;
    } else {
        CUCKOO_LOG(LOG_ERROR) << "OpenFileFromRemote(): small read file remote failed";
    }
 ...
 */
int CuckooStore::CloseTmpFiles(OpenInstance *openInstance, bool isFlush, bool isSync)
{
    CUCKOO_LOG(LOG_INFO) << "CuckooStore::CloseTmpFiles() called to " << (isFlush ? "flush" : "close") << " file "
                         << openInstance->path;

    /* physical file should be opened after RW */
    if (openInstance->physicalFd == UINT64_MAX) {
        CUCKOO_LOG(LOG_WARNING) << "CuckooStore::CloseTmpFiles() fd not set";
        return 0;
    }
    /* nodeId should be allocated after RW */
    if (openInstance->nodeId == -1) {
        CUCKOO_LOG(LOG_WARNING) << "CuckooStore::CloseTmpFiles() nodeId not set";
        return 0;
    }
    ...
    if (isFlush) {
        ...
        CUCKOO_LOG(LOG_WARNING) << "CloseTmpFiles(): close called without flush";
        ret = CloseTmpFiles(openInstance, true, isSync);
        if (ret != 0) {
            CUCKOO_LOG(LOG_ERROR) << "CloseTmpFiles(): call flush in close failed";
            openInstance->writeFail = true;
        }
    }
    ...
    if (!openInstance->isRemoteCall) {
        ...
        if (completeRet != 0) {
            CUCKOO_LOG(LOG_ERROR) << "In CuckooStore::CloseTmpFiles() call complete() failed for node "
                                  << openInstance->nodeId;
            openInstance->writeFail = true;
            ret = completeRet;
        }
    ...
    if (StoreNode::GetInstance()->IsLocal(openInstance->nodeId)) {
        ...
        if (openInstance->writeCnt > 0 && !openInstance->writeFail) {
            if (isSync) {
                ...
                CUCKOO_LOG(LOG_INFO) << "CloseTmpFiles(): file " << openInstance->path << " fsync-ed";
            }
...
int CuckooStore::FlushToStorage(std::string path, uint64_t inodeId)
{
    int ret = 0;
    ...
    if (ret == 0) {
        CUCKOO_LOG(LOG_INFO) << "Flush file " << object << " to obs succeeded!";
    } else {
        CUCKOO_LOG(LOG_ERROR) << "Flush file " << object << " to obs failed!";
    }
 ...
 */
int CuckooStore::ReadSmallFiles(OpenInstance *openInstance)
{
    uint64_t inodeId = openInstance->inodeId;
    size_t bufSize = openInstance->readBufferSize;
    ...
    if (DiskCache::GetInstance().Find(inodeId, true)) {
        ...
        if (localFd < 0) {
            int err = errno;
            CUCKOO_LOG(LOG_ERROR) << "ReadSmallFiles(): open file failed : " << fileName << " << " << strerror(errno);
            DiskCache::GetInstance().Unpin(inodeId);
            return -err;
        }
        CuckooStats::GetInstance().stats[BLOCKCACHE_READ] += bufSize;
        ...
        if (retSize != (ssize_t)bufSize) {
            ...
            CUCKOO_LOG(LOG_ERROR) << "ReadSmallFiles(): Pread size is not equal to size: " << strerror(err);
            close(localFd);
            DiskCache::GetInstance().Unpin(inodeId);
            return -err;
        }
        ...
        if (!persistToStorage) {
            CUCKOO_LOG(LOG_ERROR) << "ReadSmallFiles(): no local cache exists";
            return -ENOENT;
        }

        /* may write, sync download file from obs to file and buffer */
        if ((openInstance->oflags & O_ACCMODE) != O_RDONLY) {
            CUCKOO_LOG(LOG_INFO) << "ReadSmallFiles(): may write, sync download file from obs to file and buffer";
            bool isSync = true;
            bool toBuffer = true;
        ...
        if (openInstance->isRemoteCall) {
            CUCKOO_LOG(LOG_INFO) << "ReadSmallFiles(): remote call, bg load obs and return failure";
        ...
        if (ret < 0) {
            CUCKOO_LOG(LOG_ERROR) << "Obs read failed";
            return -EIO;
        }
 ...
 */
int CuckooStore::WriteToFileAsync(uint64_t inodeId, std::string &fileName, std::shared_ptr<char> buf, size_t bufSize)
{
    ...
    if (!lockerPtr->isLocked()) {
        CUCKOO_LOG(LOG_INFO) << "WriteToFileAsync(): No need to write local file, other acquired the lock, abort";
        return 0;
    }

    if (DiskCache::GetInstance().Find(inodeId, false)) {
        CUCKOO_LOG(LOG_INFO) << "WriteToFileAsync(): No need to write local file, other created local file, abort";
        return 0;
    }
    if (!DiskCache::GetInstance().PreAllocSpace(bufSize)) {
        CUCKOO_LOG(LOG_ERROR) << "WriteToFileAsync(): Can not pre-allocate enough space!";
        return -ENOSPC;
    }
    ...
    if (fd < 0) {
        int err = errno;
        CUCKOO_LOG(LOG_ERROR) << "WriteToFileAsync(): open file failed : " << fileName << ", " << strerror(errno);
    ...
    task.task = [fd, buf, bufSize, inodeId, lockerPtr]() {
        CuckooStats::GetInstance().stats[BLOCKCACHE_WRITE] += bufSize;
        int retSize = pwrite(fd, buf.get(), bufSize, 0);
        int err = errno;
        close(fd);
        if (retSize < 0) {
            CUCKOO_LOG(LOG_ERROR) << "WriteToFileAsync(): pwrite failed : " << strerror(err);
        } else {
 ...
 */
int CuckooStore::ReadSmallFilesForBrpc(uint64_t inodeId,
                                       ...
                                       bool nodeFail)
{
    ...
    if (DiskCache::GetInstance().Find(inodeId, true)) {
        ...
        if (localFd < 0) {
            int err = errno;
            CUCKOO_LOG(LOG_ERROR) << "ReadSmallFilesForBrpc(): open file " << fileName
                                  << " failed : " << strerror(errno);
            DiskCache::GetInstance().Unpin(inodeId);
            return -err;
        }
        CuckooStats::GetInstance().stats[BLOCKCACHE_READ] += size;
        ...
        if (retSize != (ssize_t)size) {
            ...
            CUCKOO_LOG(LOG_ERROR) << "ReadSmallFilesForBrpc(): Pread size not equal: " << strerror(err);
            close(localFd);
            DiskCache::GetInstance().Unpin(inodeId);
            return -err;
        }
        ...
        /* Cache Miss: load file from obs */
        if (!persistToStorage) {
            CUCKOO_LOG(LOG_ERROR) << "ReadSmallFilesForBrpc(): no local cache exists";
            return -ENOENT;
        }
        ...
        CUCKOO_LOG(LOG_INFO) << "ReadSmallFilesForBrpc(): remote call, bg load obs and return failure";
        // bg load obs and return failure
        bool isSync = false;
 ...
 */
int CuckooStore::DownLoadFromStorageForBrpc(uint64_t inodeId,
                                            const std::string &path,
                                            char *buf,
                                            size_t bufSize,
                                            bool isSync,
...
int CuckooStore::DeleteFiles(uint64_t inodeId, int nodeId, std::string path)
{
    int ret = 0;
    if (nodeId == -1 || StoreNode::GetInstance()->IsLocal(nodeId)) {
        if (DiskCache::GetInstance().Find(inodeId, false)) {
            ...
            CUCKOO_LOG(LOG_ERROR) << "Delete file " << GetFilePath(inodeId) << " failed : " << strerror(ENOENT);
            return -ENOENT;
        }
    } else {
        /* remote file to delete */
        std::shared_ptr<CuckooIOClient> cuckooIOClient = StoreNode::GetInstance()->GetRpcConnection(nodeId);
        if (cuckooIOClient != nullptr) {
            ret = cuckooIOClient->DeleteFile(inodeId, nodeId, path);
            if (ret != 0) {
                CUCKOO_LOG(LOG_ERROR) << "Delete remote file failed : " << strerror(-ret) << ", for node " << nodeId;
            } else {
                return ret;
            }
        } else {
    ...
    if (persistToStorage) {
        ...
        if (ret != 0) {
            CUCKOO_LOG(LOG_ERROR) << "delete file from obs failed! ";
            return -EIO;
        }
    }
    return ret;
...
int CuckooStore::StatFS(struct statvfs *vfsbuf)
{
    if (persistToStorage) {
    ...
    if (ret != 0) {
        int err = errno;
        CUCKOO_LOG(LOG_ERROR) << "StatFS failed: " << strerror(errno);
        return -err;
    }
    ...
    for (int nodeId : nodeVector) {
        ...
        std::shared_ptr<CuckooIOClient> cuckooIOClient = StoreNode::GetInstance()->GetRpcConnection(nodeId);
        // cuckooIOClient shouldn't be null
        if (cuckooIOClient != nullptr) {
            ...
            ret = cuckooIOClient->StatFS(rpcEndpoint, &remoteVfsBuf);
            if (ret != 0) {
                CUCKOO_LOG(LOG_ERROR) << "statFS remote failed: " << strerror(-ret) << ", for node " << nodeId;
                continue;
            }
...
int CuckooStore::StatFsStorage(struct statvfs *vfsbuf) { return storage->StatFs(vfsbuf); }

int CuckooStore::StatFSForBrpc(const std::string &path,
                               uint64_t &fblocks,
                               uint64_t &fbfree,
                               uint64_t &fbavail,
                               uint64_t &ffiles,
...
int CuckooStore::CopyData(const std::string &srcName, const std::string &dstName)
{
...
int CuckooStore::DeleteDataAfterRename(const std::string &objectName)
{
    return storage->DeleteObject(objectName.substr(1));
}

/* O_TRUNC must be called with O_WR */
int CuckooStore::TruncateFile(OpenInstance *openInstance, off_t size)
{
    CUCKOO_LOG(LOG_INFO) << "CuckooStore::TruncateFile() called by " << (openInstance->isRemoteCall ? "rpc" : "fuse")
    ...
    if (!openInstance->isOpened.load()) {
        ...
        ret = OpenFile(openInstance);
        if (ret != 0) {
            CUCKOO_LOG(LOG_ERROR) << "In TruncateFile(): OpenFile() failed";
            return ret;
        }
        openInstance->isOpened = true;
    }
    ...
    if (StoreNode::GetInstance()->IsLocal(openInstance->nodeId)) {
        ...
        if (ret != 0) {
            int err = errno;
            std::string fileName = GetFilePath(openInstance->inodeId);
            CUCKOO_LOG(LOG_ERROR) << "Truncate file " << fileName << " failed : " << strerror(err);
            return -err;
        }
    } else {
        ...
        std::shared_ptr<CuckooIOClient> cuckooIOClient =
            StoreNode::GetInstance()->GetRpcConnection(openInstance->nodeId);
        if (cuckooIOClient != nullptr) {
            ret = cuckooIOClient->TruncateFile(openInstance->physicalFd, size);
            if (ret != 0) {
                CUCKOO_LOG(LOG_ERROR) << "Truncate remote file failed : " << strerror(-ret) << ", for node "
                                      << openInstance->nodeId;
                return ret;
            }
        } else {
...
int CuckooStore::TruncateOpenInstance(OpenInstance *openInstance, off_t size)
{
    int ret = 0;
    ...
    if (openInstance->writeStream.GetSize() > 0) {
        ...
        CUCKOO_LOG(LOG_INFO) << "In TruncateOpenInstance(): Persisting the written";
        ret = openInstance->writeStream.Complete(openInstance->currentSize.load(), true, false);
        if (ret != 0) {
            CUCKOO_LOG(LOG_ERROR) << "In TruncateOpenInstance(): persist written before truncate failed";
            return ret;
        }
    }
    ...
    if (!StoreNode::GetInstance()->IsLocal(openInstance->nodeId) && openInstance->isOpened.load()) {
        ...
        std::shared_ptr<CuckooIOClient> cuckooIOClient =
            StoreNode::GetInstance()->GetRpcConnection(openInstance->nodeId);
        if (cuckooIOClient != nullptr) {
            int ret = cuckooIOClient->TruncateOpenInstance(openInstance->physicalFd, size);
            if (ret != 0) {
                CUCKOO_LOG(LOG_ERROR) << "Truncate remote openInstance failed : " << strerror(-ret) << ", for node "
                                      << openInstance->nodeId;
                return ret;
            }
        }
