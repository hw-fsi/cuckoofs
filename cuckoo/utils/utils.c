...
void GetRelationOid(const char *relationName, Oid *relOid)
{
    ...
    if (*relOid == InvalidOid) {
        *relOid = get_relname_relid(relationName, PG_CATALOG_NAMESPACE);
        ...
        if (*relOid == InvalidOid) {
            CUCKOO_ELOG_ERROR_EXTENDED(PROGRAM_ERROR, "GetRelationOid failed for %s!", relationName);
        }
    }
}
...
ScanKeyData ForeignServerTableScanKey[LAST_CUCKOO_FOREIGN_SERVER_TABLE_SCANKEY_TYPE];
static void InitializeForeignServerTableScanCache(void)
{
    ...
    ForeignServerTableScanKey[FOREIGN_SERVER_TABLE_SERVER_ID_EQ].sk_attno = Anum_cuckoo_foreign_server_server_id;
}

ScanKeyData DirectoryTableScanKey[LAST_CUCKOO_DIRECTORY_TABLE_SCANKEY_TYPE];
static void InitializeDirectoryTableScanCache(void)
{
    ...
    DirectoryTableScanKey[DIRECTORY_TABLE_PARENT_ID_EQ].sk_attno = Anum_cuckoo_directory_table_parent_id;
    ...
    DirectoryTableScanKey[DIRECTORY_TABLE_NAME_EQ].sk_attno = Anum_cuckoo_directory_table_name;
}

ScanKeyData InodeTableScanKey[LAST_CUCKOO_INODE_TABLE_SCANKEY_TYPE];
static void InitializeInodeTableScanCache(void)
{
...
ScanKeyData InodeTableIndexParentIdPartIdNameScanKey[LAST_CUCKOO_INODE_TABLE_INDEX_PARENT_ID_PART_ID_NAME_SCANKEY_TYPE];
static void InitializeInodeTableIndexParentIdPartIdNameScanCache(void)
{
...
ScanKeyData XattrTableScanKey[LAST_CUCKOO_XATTR_TABLE_SCANKEY_TYPE];
static void InitializeXattrTableScanCache(void)
{
    ...
    XattrTableScanKey[XATTR_TABLE_PARENT_ID_PART_ID_GT].sk_attno = Anum_cuckoo_xattr_table_parentid_partid;
    ...
    XattrTableScanKey[XATTR_TABLE_PARENT_ID_PART_ID_GE].sk_attno = Anum_cuckoo_xattr_table_parentid_partid;
    ...
    XattrTableScanKey[XATTR_TABLE_PARENT_ID_PART_ID_LE].sk_attno = Anum_cuckoo_xattr_table_parentid_partid;
    ...
    XattrTableScanKey[XATTR_TABLE_PARENT_ID_PART_ID_EQ].sk_attno = Anum_cuckoo_xattr_table_parentid_partid;
    ...
    XattrTableScanKey[XATTR_TABLE_NAME_GT].sk_attno = Anum_cuckoo_xattr_table_name;
    ...
    XattrTableScanKey[XATTR_TABLE_NAME_EQ].sk_attno = Anum_cuckoo_xattr_table_name;
    ...
    XattrTableScanKey[XATTR_TABLE_XKEY_GT].sk_attno = Anum_cuckoo_xattr_table_xkey;
    ...
    XattrTableScanKey[XATTR_TABLE_XKEY_EQ].sk_attno = Anum_cuckoo_xattr_table_xkey;
}
...
bool ArrayTypeArrayToDatumArrayAndSize(ArrayType *arrayObject, Datum **datumArray, int *datumArrayLength)
{
    ...
    if (arrayHasNull) {
        CUCKOO_ELOG_ERROR(ARGUMENT_ERROR, "array object cannot contain null values");
    }
...
Oid CuckooExtensionOwner(void)
{
    static Oid cuckooOwner = InvalidOid;
    ...
    ScanKeyInit(&key[0], Anum_pg_extension_extname, BTEqualStrategyNumber, F_NAMEEQ, CStringGetDatum("cuckoo"));
    ...
    if (!HeapTupleIsValid(extTup))
        ereport(ERROR, (errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE), errmsg("extension cuckoo does not exist")));

    /*
     * get the cuckoo extension owner
     */
    ...
    if (!superuser_arg(extOwner)) {
        ereport(ERROR,
                (errcode(ERRCODE_INSUFFICIENT_PRIVILEGE), errmsg("cuckoo extension's owner should be superuser")));
    }

    cuckooOwner = extOwner;
