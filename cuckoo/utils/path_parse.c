...
{
    if (!path || path[0] == '\0')
        CUCKOO_ELOG_ERROR(ARGUMENT_ERROR, "path is null.");
    if (path[0] != '/')
        CUCKOO_ELOG_ERROR(ARGUMENT_ERROR, "path should start with /.");
...
void PathParseMemoryContextInit()
{
    PathParseContext = AllocSetContextCreate(TopMemoryContext,
                                             "cuckoo path parse memory context",
...
CuckooErrorCode PathParseTreeInsert(PathParseTree root,
...
CuckooErrorCode VerifyPathValidity(const char *path, int32_t requirement, int32_t *property)
{
    *property = 0;
...
uint64_t GetNextSequenceNum()
{
    ...
    if (remainNumberCount == 0) {
        ...
        SetUserIdAndSecContext(CuckooExtensionOwner(), SECURITY_LOCAL_USERID_CHANGE);

        sequenceNumDatum = DirectFunctionCall1(nextval_oid, sequenceIdDatum);

        SetUserIdAndSecContext(savedUserId, savedSecurityContext);

        remainNumberCount = 32; // same with defs in cuckoo.pg_dfs_inodeid_seq
