...
{
    CuckooSupportMetaService metaService;
    int count;
    union
    {
...
static int
CuckooAppendRemoteCommandToWorkerList(RemoteCommand *remoteCommand, uint32_t remoteCommandFlag, List *workerIdList)
{
    if (RemoteConnectionCommandCache == NULL)
...
int CuckooPlainCommandOnWorkerList(const char *command, uint32_t remoteCommandFlag, List *workerIdList)
{
    ...
    return CuckooAppendRemoteCommandToWorkerList(remoteCommand, remoteCommandFlag, workerIdList);
}
                               ...
                               List *workerIdList)
{
    if (metaService == PLAIN_COMMAND)
        CUCKOO_ELOG_ERROR(PROGRAM_ERROR, "Use CuckooPlainCommandOnWorkerList instead.");
...
static bool CheckPQpipelineSyncFinished(PGconn *conn)
{
    ...
    if (PQresultStatus(res) != PGRES_PIPELINE_SYNC)
        CUCKOO_ELOG_ERROR(PROGRAM_ERROR, "Unexpected result. There must be a PGRES_PIPELINE_SYNC.");
    res = FetchPGresultAndMark(conn);
    if (res != NULL)
        CUCKOO_ELOG_ERROR(PROGRAM_ERROR, "Unexpected result. There must be a NULL.");
    return true;
}
...
MultipleServerRemoteCommandResult CuckooSendCommandAndWaitForResult()
{
    ...
    for (int i = 0; i < list_length(connList); ++i) {
        ...
        if (foreignServerConn->transactionState == CUCKOO_REMOTE_TRANSACTION_NONE &&
            ...
            if (!PQsendQueryParams(foreignServerConn->conn, "BEGIN;", 0, NULL, NULL, NULL, NULL, 0))
                CUCKOO_ELOG_ERROR_EXTENDED(REMOTE_QUERY_FAILED,
        ...
        if (foreignServerConn->transactionState == CUCKOO_REMOTE_TRANSACTION_NONE) {
            if (remoteConnectionCommand->commandFlag & REMOTE_COMMAND_FLAG_WRITE) {
                foreignServerConn->transactionState = CUCKOO_REMOTE_TRANSACTION_BEGIN_FOR_WRITE;
            } else if (remoteConnectionCommand->commandFlag & REMOTE_COMMAND_FLAG_NEED_TRANSACTION_SNAPSHOT) {
                foreignServerConn->transactionState = CUCKOO_REMOTE_TRANSACTION_BEGIN_FOR_SNAPSHOT;
            }
        }
    ...
    for (int i = 0; i < list_length(connList); ++i) {
        ...
        if (needHandleTransactionControl) {
            ...
            res = FetchPGresultAndMark(foreignServerConn->conn);
            if (res != NULL)
                CUCKOO_ELOG_ERROR(PROGRAM_ERROR, "a NULL is expected");
        }
...
void CuckooRemoteCommandPrepare()
{
    ...
    for (int i = 0; i < list_length(workerIdList); ++i) {
        ...
        if (foreignServerConn->transactionState == CUCKOO_REMOTE_TRANSACTION_BEGIN_FOR_WRITE)
            ++writeServerCount;
    }
    ...
    for (int i = 0; i < list_length(connList); ++i) {
        ...
        if (foreignServerConn->transactionState == CUCKOO_REMOTE_TRANSACTION_BEGIN_FOR_WRITE) {
            ...
            if (!PQpipelineSync(foreignServerConn->conn))
                CUCKOO_ELOG_ERROR(REMOTE_QUERY_FAILED, "error while trying to sync pipeline.");
        }
    }
    ...
    for (int i = 0; i < list_length(workerIdList); ++i) {
        ...
        case PGRES_TUPLES_OK:
            ...
            if (foreignServerConn->transactionState == CUCKOO_REMOTE_TRANSACTION_BEGIN_FOR_WRITE) {
                foreignServerConn->transactionState = CUCKOO_REMOTE_TRANSACTION_PREPARE;
    ...
    if (errorMsg->len != 0)
        CUCKOO_ELOG_ERROR_EXTENDED(REMOTE_QUERY_FAILED, "%s", errorMsg->data);
}

void CuckooRemoteCommandCommit()
{
    if (RemoteConnectionCommandCache == NULL) // no command sent
        return;
    ...
    for (int i = 0; i < list_length(workerIdList); ++i) {
        ForeignServerConnection *foreignServerConn = list_nth(connList, i);
        if (ClearPGresultInPGconn(foreignServerConn->conn)) {
            CUCKOO_ELOG_WARNING(PROGRAM_ERROR,
        ...
        case CUCKOO_REMOTE_TRANSACTION_BEGIN_FOR_SNAPSHOT:
        case CUCKOO_REMOTE_TRANSACTION_BEGIN_FOR_WRITE:
            if (!PQsendQueryParams(foreignServerConn->conn, "COMMIT;", 0, NULL, NULL, NULL, NULL, 0)) {
                CUCKOO_ELOG_WARNING_EXTENDED(
                    REMOTE_QUERY_FAILED,
            ...
            if (!PQpipelineSync(foreignServerConn->conn)) {
                CUCKOO_ELOG_WARNING(REMOTE_QUERY_FAILED, "error while trying to sync pipeline.");
                return;
            }
            break;
        case CUCKOO_REMOTE_TRANSACTION_PREPARE:
    ...
    for (int i = 0; i < list_length(connList); ++i) {
        ...
        case PGRES_TUPLES_OK:
            res = FetchPGresultAndMark(foreignServerConn->conn);
            if (res != NULL) {
                CUCKOO_ELOG_WARNING(PROGRAM_ERROR, "a NULL is expected.");
                return;
            }
            ...
            break;
        default:
            CUCKOO_ELOG_WARNING_EXTENDED(REMOTE_QUERY_FAILED,
                                         "workerId: %d, errorMsg: %s.",
                                         foreignServerConn->serverId,
                                         PQresultErrorMessage(res));
            return;
        ...
        foreignServerConn->transactionState = CUCKOO_REMOTE_TRANSACTION_NONE;
    }
}

bool CuckooRemoteCommandAbort()
{
    if (RemoteConnectionCommandCache == NULL) // no command sent
        return true;
...
void RemoteConnectionCommandCacheInit()
{
    if (RemoteConnectionCommandCacheContext == NULL) {
        RemoteConnectionCommandCacheContext =
            AllocSetContextCreateInternal(TopMemoryContext,
                                          "Cuckoo Remote Connection Command Cache Context",
    ...
    RemoteConnectionCommandCache =
        hash_create("Cuckoo Remote Comm Command Cache Hash Table", FOREIGN_SERVER_NUM_EXPECT, &info, hashFlags);
}