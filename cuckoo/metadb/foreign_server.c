...
PG_FUNCTION_INFO_V1(cuckoo_foreign_server_test);
PG_FUNCTION_INFO_V1(cuckoo_insert_foreign_server);
PG_FUNCTION_INFO_V1(cuckoo_delete_foreign_server);
PG_FUNCTION_INFO_V1(cuckoo_update_foreign_server);
PG_FUNCTION_INFO_V1(cuckoo_reload_foreign_server_cache);

Datum cuckoo_foreign_server_test(PG_FUNCTION_ARGS)
{
    ...
    if (strcmp(mode, "EXCESS_MAX_FOREIGN_SERVER_NUM") == 0) {
        ...
        for (int i = 0; i < list_length(workerIdList); ++i) {
            ...
            FormData_cuckoo_foreign_server *foreignServerInfo = list_nth(foreignServerInfoList, i);
...
Datum cuckoo_insert_foreign_server(PG_FUNCTION_ARGS)
{
...
Datum cuckoo_delete_foreign_server(PG_FUNCTION_ARGS)
{
    int32_t serverId = PG_GETARG_INT32(0);
...
Datum cuckoo_update_foreign_server(PG_FUNCTION_ARGS)
{
    int32_t serverId = PG_GETARG_INT32(0);
...
Datum cuckoo_reload_foreign_server_cache(PG_FUNCTION_ARGS)
{
    InvalidateForeignServerShmemCache();
    ReloadForeignServerShmemCache();
...
{
    GetRelationOid("cuckoo_foreign_server", &CachedRelationOid[CACHED_RELATION_FOREIGN_SERVER]);
    return CachedRelationOid[CACHED_RELATION_FOREIGN_SERVER];
}

Oid ForeignServerRelationIndexId(void)
{
    GetRelationOid("cuckoo_foreign_server_index", &CachedRelationOid[CACHED_RELATION_FOREIGN_SERVER_INDEX]);
...
void ForeignServerCacheInit()
{
    ForeignServerContext = AllocSetContextCreateInternal(TopMemoryContext,
                                                         "Cuckoo Foreign Server Hash Table Context",
...
static inline void InsertForeignServerByRel(Relation rel,
                                            ...
                                            const char *userName)
{
    Datum values[Natts_cuckoo_foreign_server];
    bool isNulls[Natts_cuckoo_foreign_server];
    HeapTuple heapTuple;
    TupleDesc tupleDescriptor = RelationGetDescr(rel);

    memset(isNulls, false, sizeof(isNulls));
    values[Anum_cuckoo_foreign_server_server_id - 1] = Int32GetDatum(serverId);
    values[Anum_cuckoo_foreign_server_server_name - 1] = CStringGetTextDatum(serverName);
    values[Anum_cuckoo_foreign_server_host - 1] = CStringGetTextDatum(host);
    values[Anum_cuckoo_foreign_server_port - 1] = Int32GetDatum(port);
    values[Anum_cuckoo_foreign_server_is_local - 1] = BoolGetDatum(isLocal);
    values[Anum_cuckoo_foreign_server_user_name - 1] = CStringGetTextDatum(userName);
...
static inline void DeleteForeignServerByRel(Relation rel, const int32_t serverId)
{
    ...
    if (!HeapTupleIsValid(heapTuple)) {
        CUCKOO_ELOG_ERROR_EXTENDED(ARGUMENT_ERROR, "serverId " INT32_PRINT_SYMBOL " is not existed.", serverId);
    } else {
...
static inline void UpdateForeignServerByRel(Relation rel, const int32_t serverId, const char *host, const int32_t port)
{
    ...
    if (!HeapTupleIsValid(heapTuple)) {
        ...
        Datum datumArray[Natts_cuckoo_foreign_server];
        bool isNullArray[Natts_cuckoo_foreign_server];
        memset(isNullArray, 0, sizeof(isNullArray));
        bool doUpdateArray[Natts_cuckoo_foreign_server];
        memset(doUpdateArray, 0, sizeof(doUpdateArray));
        datumArray[Anum_cuckoo_foreign_server_host - 1] = CStringGetTextDatum(host);
        doUpdateArray[Anum_cuckoo_foreign_server_host - 1] = true;
        datumArray[Anum_cuckoo_foreign_server_port - 1] = Int32GetDatum(port);
        doUpdateArray[Anum_cuckoo_foreign_server_port - 1] = true;
...
static void RenewForeignServerLocalCache(const bool needLock)
{
    ...
    HASH_SEQ_STATUS status;
    ForeignServerConnection *foreignServerConnection;
    FormData_cuckoo_foreign_server *foreignServerInfo;
    hash_seq_init(&status, ForeignServerConnectionManager);
    while ((foreignServerConnection = hash_seq_search(&status)) != NULL) {
        int serverId = foreignServerConnection->serverId;
        bool found;
    ...
    if (LocalServerId == -1) {
        ...
        if (LocalServerId == -1)
            CUCKOO_ELOG_ERROR(PROGRAM_ERROR, "no server is local. check foreign_server_table.");
    }
...
List *GetForeignServerInfo(List *foreignServerIdList)
{
    ...
    for (int i = 0; i < list_length(foreignServerIdList); ++i) {
        ...
        FormData_cuckoo_foreign_server *dataCopy = palloc(sizeof(FormData_cuckoo_foreign_server));
        memcpy(dataCopy, foreignServerInfo, sizeof(FormData_cuckoo_foreign_server));
...
static bool PGconnPrepare(PGconn *conn)
{
    ...
    do {
        if (!PQenterPipelineMode(conn))
            break;
        if (!PQsendQueryParams(conn, "select cuckoo_prepare_commands();", 0, NULL, NULL, NULL, NULL, 0))
            break;
...
List *GetForeignServerConnection(List *foreignServerIdList)
{
    ...
    for (int i = 0; i < list_length(foreignServerIdList); ++i) {
        ...
        if (foreignServerConnection->conn == NULL) {
            FormData_cuckoo_foreign_server *foreignServerInfo =
                hash_search(ForeignServerShmemCache, &serverId, HASH_FIND, &found);
            if (!found) {
                CUCKOO_ELOG_ERROR_EXTENDED(PROGRAM_ERROR, "trying to ask for invalid server, server id: %d.", serverId);
            }

            foreignServerConnection->transactionState = CUCKOO_REMOTE_TRANSACTION_NONE;
    ...
    if (!connectSucceed) {
        CUCKOO_ELOG_ERROR(PROGRAM_ERROR, "error while trying to get connection.");
    }
    return result;
}
...
size_t ForeignServerShmemsize()
{
    return sizeof(ShmemControlData) + sizeof(int32_t) + sizeof(pg_atomic_uint32) +
           sizeof(FormData_cuckoo_foreign_server) * FOREIGN_SERVER_NUM_MAX;
}
void ForeignServerShmemInit()
{
    ...
    HASHCTL info;
    info.keysize = sizeof(int32_t);
    info.entrysize = sizeof(FormData_cuckoo_foreign_server);
    ForeignServerShmemCache = ShmemInitHash("Cuckoo Foreign Server Shmem Hash Table",
    ...
    if (!initialized) {
        ...
        ForeignServerShmemControl->lockTrancheName = "Cuckoo Foreign Server Control";
...
void ReloadForeignServerShmemCache()
{
    ...
    while (HeapTupleIsValid(heapTuple = systable_getnext(scanDesc))) {
        ...
        int32_t serverId = DatumGetInt32(datumArray[Anum_cuckoo_foreign_server_server_id - 1]);
        bool found;
        FormData_cuckoo_foreign_server *entry = hash_search(ForeignServerShmemCache, &serverId, HASH_ENTER, &found);

        entry->server_id = serverId;
        text_to_cstring_buffer((text *)datumArray[Anum_cuckoo_foreign_server_server_name - 1],
                               entry->server_name,
                               FOREIGN_SERVER_NAME_MAX_LENGTH);
        text_to_cstring_buffer((text *)datumArray[Anum_cuckoo_foreign_server_host - 1], entry->host, HOST_MAX_LENGTH);
        entry->port = DatumGetInt32(datumArray[Anum_cuckoo_foreign_server_port - 1]);
        entry->is_local = DatumGetBool(datumArray[Anum_cuckoo_foreign_server_is_local - 1]);
        text_to_cstring_buffer((text *)datumArray[Anum_cuckoo_foreign_server_user_name - 1],
                               entry->user_name,
                               FOREIGN_SERVER_NAME_MAX_LENGTH);
    }
    ...
    if (exceedMaxNumOfForeignServer) {
        InvalidateForeignServerShmemCache();
        CUCKOO_ELOG_ERROR_EXTENDED(PROGRAM_ERROR, "exceed max number of foreign server: %d.", FOREIGN_SERVER_NUM_MAX);
    }
