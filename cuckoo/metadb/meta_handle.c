...
static Oid GetRelationOidByName_CUCKOO(const char *relationName);
...
{
    if (GetLocalServerId() != CUCKOO_CN_SERVER_ID)
        CUCKOO_ELOG_ERROR(WRONG_WORKER, "mkdir can only be called on CN.");
    ...
    CuckooMetaCallOnWorkerList(MKDIR_SUB_MKDIR,
                               validInputIndexArraySize,
                               subMkdirParam,
    ...
    while ((entry = hash_seq_search(&status)) != 0) {
        ...
        SerializedDataMetaParamEncodeWithPerProcessFlatBufferBuilder(MKDIR_SUB_CREATE,
                                                                     ...
                                                                     &subCreateParam);
        CuckooMetaCallOnWorkerList(MKDIR_SUB_CREATE,
                                   validInputIndexArrayForSubCreateSize,
                                   subCreateParam,
                                   REMOTE_COMMAND_FLAG_WRITE,
    ...
    // 4.
    MultipleServerRemoteCommandResult totalRemoteRes = CuckooSendCommandAndWaitForResult();
    ...
    for (int i = 0; i < list_length(totalRemoteRes); ++i) {
        ...
        if (list_length(remoteRes->remoteCommandResult) < 1 || list_length(remoteRes->remoteCommandResult) > 2)
            CUCKOO_ELOG_ERROR(PROGRAM_ERROR, "unexpected. situation");
        PGresult *res = NULL;

        // 4.1
        res = list_nth(remoteRes->remoteCommandResult, 0);
        if (PQntuples(res) != 1 || PQnfields(res) != 1)
            CUCKOO_ELOG_ERROR(REMOTE_QUERY_FAILED, "PGresult is corrupt.");
        ...
        if (!SerializedDataMetaResponseDecode(MKDIR_SUB_MKDIR, validInputIndexArraySize, &subMkdirResponse, resArray))
            CUCKOO_ELOG_ERROR(ARGUMENT_ERROR, "serialized response is corrupt.");

        for (int j = 0; j < validInputIndexArraySize; ++j)
            if (resArray[j].errorCode != SUCCESS)
                CUCKOO_ELOG_ERROR(PROGRAM_ERROR,
...
void CuckooMkdirSubMkdirHandle(MetaProcessInfo *infoArray, int count)
{
    ...
    for (int i = 0; i < count; ++i) {
        ...
        if (info->parentId == -1 || info->name == NULL)
            CUCKOO_ELOG_ERROR(ARGUMENT_ERROR, "CuckooMkdirSubMkdir has received invalid input.");
        InsertDirectoryByDirectoryHashTable(directoryRel,
                                            indexState,
                                            info->parentId,
                                            info->name,
...
void CuckooMkdirSubCreateHandle(MetaProcessInfo *infoArray, int count)
{
    ...
    for (int i = 0; i < count; ++i) {
        ...
        if (info->inodeId == -1 || info->parentId_partId == -1 || info->name == NULL)
            CUCKOO_ELOG_ERROR(ARGUMENT_ERROR, "CuckooMkdirSubCreateHandle has received invalid input.");
        int shardId, workerId;
        SearchShardInfoByShardValue(info->parentId_partId, &shardId, &workerId);
        if (workerId != GetLocalServerId())
            CUCKOO_ELOG_ERROR(ARGUMENT_ERROR, "CuckooMkdirSubCreate has received invalid input.");
    ...
    while ((entry = hash_seq_search(&status)) != 0) {
        Relation workerInodeRel =
            table_open(GetRelationOidByName_CUCKOO(GetInodeShardName(entry->shardId)->data), RowExclusiveLock);
...
void CuckooCreateHandle(MetaProcessInfo *infoArray, int count, bool updateExisted)
{
    ...
    for (int i = 0; i < count; ++i) {
        ...
        CuckooErrorCode errorCode = PathParseTreeInsert(NULL,
                                                        directoryRel,
                                                        info->path,
    ...
    while ((entry = hash_seq_search(&status)) != 0) {
        ...
        Oid inodeIndexOid = GetRelationOidByName_CUCKOO(inodeIndexShardName->data);
        ...
        while (list_length(toHandleMetaProcessList) != 0) {
            ...
            Relation workerInodeRel = table_open(GetRelationOidByName_CUCKOO(inodeShardName->data), RowExclusiveLock);
            CatalogIndexState indexState = CatalogOpenIndexes(workerInodeRel);
            PG_TRY();
            {
...
void CuckooStatHandle(MetaProcessInfo *infoArray, int count)
{
    for (int i = 0; i < count; ++i) {
        ...
        int32_t property;
        //
        CuckooErrorCode errorCode = VerifyPathValidity(info->path, 0, &property);
        CHECK_ERROR_CODE_WITH_CONTINUE(errorCode);
    }
    pg_qsort(infoArray, count, sizeof(MetaProcessInfo), pg_qsort_meta_process_info_by_path_cmp);
    ...
    while ((entry = hash_seq_search(&status)) != NULL) {
        StringInfo inodeShardName = GetInodeShardName(entry->shardId);
        StringInfo inodeIndexShardName = GetInodeIndexShardName(entry->shardId);
        Relation workerInodeRel = table_open(GetRelationOidByName_CUCKOO(inodeShardName->data), AccessShareLock);
        Oid workerInodeIndexOid = GetRelationOidByName_CUCKOO(inodeIndexShardName->data);
...
void CuckooReadDirHandle(MetaProcessInfo info)
{
    ...
    if (errorCode != SUCCESS)
        CUCKOO_ELOG_ERROR(errorCode, "path parse error.");
    table_close(directoryRel, AccessShareLock);
    ...
    while (shardIndex < shardTableCount) {
        int workerId = ((FormData_cuckoo_shard_table *)list_nth(shardTableData, shardIndex))->server_id;
        int shardId = ((FormData_cuckoo_shard_table *)list_nth(shardTableData, shardIndex))->range_point;
        if (workerId != GetLocalServerId()) {
            ++shardIndex;
            continue;
        }
        ...
        case NEW_SHARD:
            ...
            break;
        default:
            CUCKOO_ELOG_ERROR(PROGRAM_ERROR, "wrong state in CuckooReadDirHandle.");
        }

        Relation workerInodeRel = table_open(GetRelationOidByName_CUCKOO(inodeShardName->data), AccessShareLock);
        SysScanDesc scanDescriptor = systable_beginscan(workerInodeRel,
                                                        GetRelationOidByName_CUCKOO(inodeIndexShardName->data),
                                                        true,
                                                        GetTransactionSnapshot(),
                                                        scanKeyCount,
                                                        scanKey);
...
void CuckooRmdirHandle(MetaProcessInfo info)
{
    ...
    if (GetLocalServerId() != CUCKOO_CN_SERVER_ID)
        CUCKOO_ELOG_ERROR(WRONG_WORKER, "rmdir can only be called on CN.");

    // 1.
    int32_t property;
    CuckooErrorCode errorCode = VerifyPathValidity(path, VERIFY_PATH_VALIDITY_REQUIREMENT_MUST_BE_DIRECTORY, &property);
    if (errorCode != SUCCESS)
        CUCKOO_ELOG_ERROR(errorCode, "path verify error.");
    ...
    List *foreignServerIdList = GetAllForeignServerId(true, false);
    CuckooMetaCallOnWorkerList(RMDIR_SUB_RMDIR, 1, subRmdirParam, REMOTE_COMMAND_FLAG_WRITE, foreignServerIdList);
    ...
    CuckooMetaCallOnWorkerList(RMDIR_SUB_UNLINK,
                               1,
                               subUnlinkParam,
                               REMOTE_COMMAND_FLAG_WRITE,
...
void CuckooRmdirSubRmdirHandle(MetaProcessInfo info)
{
    ...
    if (directoryId == DIR_HASH_TABLE_PATH_NOT_EXIST)
        CUCKOO_ELOG_ERROR(FILE_NOT_EXISTS, "CuckooRmdirSubRmdirHandle: unexpected.");
    ...
    for (int i = 0; i < list_length(shardTableData); ++i) {
        int32_t workerId = ((FormData_cuckoo_shard_table *)list_nth(shardTableData, i))->server_id;
        int32_t shardId = ((FormData_cuckoo_shard_table *)list_nth(shardTableData, i))->range_point;
        if (workerId != GetLocalServerId())
            continue;
        ...
        HeapTuple heapTuple;
        while (HeapTupleIsValid(heapTuple = systable_getnext(scanDescriptor)))
            CUCKOO_ELOG_ERROR_EXTENDED(ARGUMENT_ERROR,
...
void CuckooRmdirSubUnlinkHandle(MetaProcessInfo info)
{
    ...
    SearchShardInfoByShardValue(parentId_partId, &shardId, &workerId);
    if (workerId != GetLocalServerId())
        CUCKOO_ELOG_ERROR(ARGUMENT_ERROR, "CuckooRmdirSubUnlinkHandle has received invalid input.");
    ...
    bool fileExist = SearchAndUpdateInodeTableInfo(inodeShardName->data,
                                                   ...
                                                   NULL);
    if (nlink != 1)
        CUCKOO_ELOG_ERROR(PROGRAM_ERROR, "unexpected.");
    if (!fileExist)
        CUCKOO_ELOG_ERROR_EXTENDED(FILE_NOT_EXISTS,
...
void CuckooRenameHandle(MetaProcessInfo info)
{
    ...
    if (GetLocalServerId() != 0)
        CUCKOO_ELOG_ERROR(WRONG_WORKER, "rename can only be called on CN.");
    ...
    if (renameDirectory && !(srcProperty & VERIFY_PATH_VALIDITY_PROPERTY_CAN_BE_DIRECTORY))
        CUCKOO_ELOG_ERROR(ARGUMENT_ERROR, "src is expected to be file, but it seems to be directory.");
    if (!renameDirectory && !(srcProperty & VERIFY_PATH_VALIDITY_PROPERTY_CAN_BE_FILE))
        CUCKOO_ELOG_ERROR(ARGUMENT_ERROR, "src is expected to be directory, but it seems to be file.");
    ...
    CuckooMetaCallOnWorkerList(RENAME_SUB_RENAME_LOCALLY,
                               1,
                               subRenameLocallyParam,
                               REMOTE_COMMAND_FLAG_WRITE,
    ...
    for (int i = 0; i < list_length(totalRemoteRes); ++i) {
        ...
        if (srcWorkerId != dstWorkerId && serverId == srcWorkerId) {
            ...
            SerializedDataMetaParamEncodeWithPerProcessFlatBufferBuilder(RENAME_SUB_CREATE,
                                                                         ...
                                                                         1,
                                                                         &subCreateParam);
            CuckooMetaCallOnWorkerList(RENAME_SUB_CREATE,
                                       1,
                                       subCreateParam,
                                       REMOTE_COMMAND_FLAG_WRITE,
    ...
    if (srcWorkerId != dstWorkerId) {
        totalRemoteRes = CuckooSendCommandAndWaitForResult();
        if (list_length(totalRemoteRes) != 1)
            CUCKOO_ELOG_ERROR(PROGRAM_ERROR, "unexpected situation 1.");
        RemoteCommandResultPerServerData *remoteRes = list_nth(totalRemoteRes, 0);
        if (remoteRes->serverId != dstWorkerId)
            CUCKOO_ELOG_ERROR(PROGRAM_ERROR, "unexpected situation 2.");
        if (list_length(remoteRes->remoteCommandResult) != 1)
            CUCKOO_ELOG_ERROR(PROGRAM_ERROR, "unexpected situation 3.");
...
void CuckooRenameSubRenameLocallyHandle(MetaProcessInfo info)
{
    ...
    if (info->targetIsDirectory) {
        if (info->srcLockOrder < 0 || info->srcLockOrder > 1)
            CUCKOO_ELOG_ERROR(ARGUMENT_ERROR, "no expected input.");
    ...
    if (srcWorkerId != GetLocalServerId())
        CUCKOO_ELOG_ERROR(WRONG_WORKER, "wrong worker.");
    ...
    scanKey[1] = InodeTableScanKey[INODE_TABLE_NAME_EQ];
    scanKey[1].sk_argument = CStringGetTextDatum(info->name);
    Relation srcInodeRel = table_open(GetRelationOidByName_CUCKOO(srcInodeShardName->data), RowExclusiveLock);
    SysScanDesc scanDescriptor = systable_beginscan(srcInodeRel,
                                                    GetRelationOidByName_CUCKOO(srcInodeIndexShardName->data),
                                                    true,
                                                    GetTransactionSnapshot(),
                                                    2,
                                                    scanKey);
    ...
    if (!HeapTupleIsValid(heapTuple))
        CUCKOO_ELOG_ERROR(FILE_NOT_EXISTS, "unexpected.");
    ...
    if (info->dstParentIdPartId != 0) {
        ...
        Relation dstInodeRel = table_open(GetRelationOidByName_CUCKOO(dstInodeShardName->data), RowExclusiveLock);
...
void CuckooUtimeNsHandle(MetaProcessInfo info)
{
    ...
    bool fileExist = SearchAndUpdateInodeTableInfo(inodeShardName->data,
                                                   ...
                                                   NULL);
    if (!fileExist)
        CUCKOO_ELOG_ERROR(FILE_NOT_EXISTS, "file doesn't exist.");
...
static Oid GetRelationOidByName_CUCKOO(const char *relationName)
{
    ...
    if (res == InvalidOid) {
        CUCKOO_ELOG_ERROR_EXTENDED(PROGRAM_ERROR, "cannot find relation %s.", relationName);
    }
    return res;
}
                                          ...
                                          int32_t *backupNodeId)
{
    ...
    if (!workerInodeRelation) {
        workerInodeRel = table_open(GetRelationOidByName_CUCKOO(workerInodeRelationName),
                                    doUpdate ? RowExclusiveLock : AccessShareLock);
    }

    if (workerInodeIndexOid == InvalidOid)
        workerInodeIndexOid = GetRelationOidByName_CUCKOO(workerInodeRelationIndexName);
    scanDescriptor =
