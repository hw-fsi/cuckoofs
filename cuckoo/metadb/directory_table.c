...
const char *DirectoryTableName = "cuckoo_directory_table";
const char *DirectoryTableIndexName = "cuckoo_directory_table_index";
...
void SearchDirectoryTableInfo(Relation directoryRel, uint64_t parentId, const char *name, uint64_t *inodeId)
{
    ...
    if (!HeapTupleIsValid(heapTuple)) {
        *inodeId = -1;
        ...
        bool isNull = false;
        *inodeId = DatumGetUInt64(heap_getattr(heapTuple, Anum_cuckoo_directory_table_inode_id, tupleDesc, &isNull));
    }
...
void InsertIntoDirectoryTable(Relation directoryRel,
                              ...
                              uint64_t inodeId)
{
    Datum values[Natts_cuckoo_directory_table];
    bool isNulls[Natts_cuckoo_directory_table];
    memset(values, 0, sizeof(values));
    memset(isNulls, false, sizeof(isNulls));
    values[Anum_cuckoo_directory_table_parent_id - 1] = UInt64GetDatum(parentId);
    values[Anum_cuckoo_directory_table_name - 1] = CStringGetTextDatum(name);
    values[Anum_cuckoo_directory_table_inode_id - 1] = UInt64GetDatum(inodeId);
...
void DeleteFromDirectoryTable(Relation directoryRel, uint64_t parentId, const char *name)
{
    ...
    HeapTuple heapTuple = systable_getnext(scanDescriptor);
    if (!HeapTupleIsValid(heapTuple)) {
        CUCKOO_ELOG_ERROR_EXTENDED(ARGUMENT_ERROR, "can not find " UINT64_PRINT_SYMBOL ":%s in disk.", parentId, name);
    } else {
