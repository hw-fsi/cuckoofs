...
def clear_liveness_file():
    ...
    shell.exec_cmd('> /home/cuckooMeta/check_liveness.sh')

def restore_liveness_file():
    content = '''#!/bin/bash
pg_isready -d postgres -U cuckooMeta --timeout=5 --quiet
if [ $? != 0 ]; then
    exit 1;
fi
if [ $? != 0 ]; then
...
if [ "${isMonitor}" = "0" ]; then
    ...
    '''
    with open('/home/cuckooMeta/check_liveness.sh', 'w') as f:
        f.write(content)
...
def update_node_table(cnhost, cnport, user, nodeid, ip, port):
    ...
    port = str(port)
    cnport = str(cnport)
    cn_connection_string = 'host={} port={} user={} dbname=postgres'.format(cnhost, cnport, user)
    logging.getLogger('logger').info('Update node table: {} {} {} {} {}'.format(nodeid, ip, port, cnhost, cnport))
    sql = "SELECT * FROM cuckoo_update_foreign_server({}, '{}', {});".format(nodeid, ip, port)
    res = execute(cn_connection_string, sql)
    return res

def update_start_background_service(host, port, user):
    port = str(port)
    connection_string = 'host={} port={} user={} dbname=postgres'.format(host, port, user)
    logging.getLogger('logger').info('Update start background service: {} {} {}'.format(host, port, user))
    sql = "SELECT * FROM cuckoo_start_background_service();"
    res = execute(connection_string, sql)
    return res

def reload_foreign_server_cache(cnhost, cnport, user):
    cnport = str(cnport)
    connection_string = 'host={} port={} user={} dbname=postgres'.format(cnhost, cnport, user)
    logging.getLogger('logger').info('Reload foreign server cache: {} {} {}'.format(cnhost, cnport, user))
    sql = "SELECT * FROM cuckoo_reload_foreign_server_cache();"
    res = execute(connection_string, sql)
    return res
...
def pg_basebackup(data_dir, host, port, user):
    ...
    shell.exec_cmd("pg_basebackup -D {} -Fp -Pv -Xs -R -h {} -p {} -U {}".format(data_dir, host, port, user))

def pg_rewind(data_dir, host, port, user):
    port = str(port)
    connection_string = 'host={} port={} user={} dbname=postgres'.format(host, port, user)
    shell.exec_cmd("pg_rewind -D {} --source-server='{}'".format(data_dir, connection_string))
    ...
    shell.exec_cmd("echo \"primary_conninfo = '{}'\" >> {}".format(connection_string, os.path.join(data_dir, 'postgresql.auto.conf')))

def is_standby_ready(connection_string):
    status, err = try_fetch_one(connection_string, "SELECT status FROM pg_stat_wal_receiver;")
...
def do_demote(data_dir, host, port, user, local_host, local_port):
    ...
    connection_string = 'host={} port={} user={} dbname=postgres'.format(local_host, local_port, user)
    ...
    if ready:
        ...
        return
    while not ready:
        logging.getLogger('logger').info('DB is not ready, Demote the DB using pg_basebackup now')
        pg_stop(data_dir)
...
def change_following_leader(data_dir, leader_host, leader_port, user):
    ...
    connection_string = 'host={} port={} user={} dbname=postgres'.format(leader_host, leader_port, user)
    logging.getLogger('logger').info('Change following leader, the new leader is {}'.format(leader_host))
    shell.exec_cmd("> {}".format(os.path.join(data_dir, 'postgresql.auto.conf')))
    shell.exec_cmd("echo \"primary_conninfo = '{}'\" >> {}".format(connection_string, os.path.join(data_dir, 'postgresql.auto.conf')))
    pg_reload(data_dir)
...
def get_lsn(host, port, user):
    ...
    sql_get_lsn_cuckoo = "SELECT * FROM pg_last_wal_receive_lsn_for_cuckoo();"
    port = str(port)
    connection_string = 'host={} port={} user={} dbname=postgres'.format(host, port, user)
    lsn = get_receive_lsn(connection_string, sql_get_lsn)
    lsn_cuckoo = get_receive_lsn(connection_string, sql_get_lsn_cuckoo)
    max_lsn = max(lsn_to_num(lsn), lsn_to_num(lsn_cuckoo))
    return max_lsn
...
def demote_for_start(data_dir, host, port, user):
    ...
    pg_start(data_dir, "~/logfile")
    restore_liveness_file()
    logging.getLogger('logger').info('Demote the DB using pg_basebackup for start successfully.')
