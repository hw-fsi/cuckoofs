...
def init_filesystem(leader_infos, user):
    ...
    create_extension = "CREATE EXTENSION cuckoo;"
    ...
    for i in range(server_num):
        ...
        for j in range(server_num):
            ...
            if i == j:
                ...
                is_local = "false"
            insert_server_sql = "SELECT cuckoo_insert_foreign_server({}, '{}', '{}', {}, {}, '{}');".format(id, name, ip, port, is_local, user)
    ...
    for i in range(server_num):
        ...
        build_shard_map_sql = "SELECT cuckoo_build_shard_table({});SELECT cuckoo_create_distributed_data_table();SELECT cuckoo_start_background_service();".format(shard_count)
        conn = psycopg2.connect(host=ip, port=port, user=user, database="postgres")
        with conn:
            cursor = conn.cursor()
            cursor.execute(build_shard_map_sql)
            conn.commit()
    mkdir_sql = "SELECT * from cuckoo_plain_mkdir('/');"
