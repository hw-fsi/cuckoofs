...
class CuckooCM:
    def __init__(self, is_cn):
        ...
        self.init_cuckoo_env()
        self._dn_path = self._root_path + "/cuckoo_dns"
        self._cn_path = self._root_path + "/cuckoo_cn"
        self._cluster_path = self._root_path + "/cuckoo_clusters"
    ...
    def init_cuckoo_env(self):
        self._hosts = os.environ.get("zk_endpoint")
        self._root_path = os.environ.get("cluster_name", "/cuckoo")
        self._user_name = os.environ.get("user_name", "cuckooMeta")
        ...
        self._data_dir = os.environ.get("data_dir", "/home/cuckooMeta/data")
    ...
    def register_node(self):
        ...
        if self._is_cn:
            self._zk_client.create(self._cn_path + "/" + self._host_node_name, value=str.encode(node_info), ephemeral=True)
        else:
            self._zk_client.create(self._dn_path + "/" + self._host_node_name, value=str.encode(node_info), ephemeral=True)
    ...
    def init_sys(self):
        ...
        try:
            ...
            try:
                ...
                self.logger.error("Failed to init the path for store the original cn and dn: {}".format(e))
                raise e
    ...
    def init_all_path(self):
        if not self._zk_client.exists(self._cluster_path):
            ...
            self._zk_client.create(self._store_node_path + "/storeNodeStatus", value=str.encode('0'))
    ...
    def leader_select(self):
        ...
        last_leader_path = "{}/{}/lastLeader".format(self._cluster_path, self._cluster_name)
        if self._zk_client.exists(self._root_path + "/ready"):
            ...
            if last_leader == self._host_node_name:
                ...
                try:
                    self._zk_client.create(leader_path, value=str.encode(node_info), ephemeral=True)
                    self._zk_client.set(last_leader_path, value=str.encode(self._host_node_name))
            ...
            if self._zk_client.exists(leader_path):
                ...
                try:
                    ...
                    self.logger.error("Failed to set the leader for the node: {}".format(e))
                    self._is_leader = False
    ...
    def write_replica(self):
        ...
        if self._is_cn:
            found = False
            while not found:
                host_node_path = "{}/{}/hostNodes".format(self._cluster_path, self._cluster_name)
                ...
                if self._host_node_name in host_nodes_info:
                    self.logger.info("The node {} is already in the cluster, write the replica".format(self._host_node_name))
                    found = True
                    break
                else:
                    if len(host_nodes_info) == self._replica_server_num + 1 and not self._zk_client.exists(self._cn_supplement_path+"/"+self._host_node_name):
                        #add it to the supplement cluster
                        self._zk_client.create("{}/{}".format(self._cn_supplement_path, self._host_node_name), ephemeral=True)
                        self.logger.info("The node {} is not in any cluster, add it to the supplement cluster".format(self._host_node_name))
                        postgresql.clean_for_supplement(self._pod_ip, self._meta_port, self._user_name)
                    time.sleep(30)
        node_info = "{}:{}".format(self._pod_ip, self._meta_port)
        replica_path = "{}/{}/replicas/{}".format(self._cluster_path, self._cluster_name, node_info)
        membership_path = "{}/{}/membership/{}".format(self._cluster_path, self._cluster_name, self._host_node_name)
        leader_path = "{}/{}".format(self._leader_path, self._cluster_name)
        while not self._zk_client.exists(leader_path):
            time.sleep(1)
        ...
        if self._zk_client.exists(self._root_path + "/ready"):
            host_path = "{}/{}/hostNodes/{}".format(self._cluster_path, self._cluster_name, self._host_node_name)
            if self._zk_client.exists(host_path):
                self.logger.info("The node {} is already in the cluster {}, write the replica".format(self._host_node_name, self._cluster_name))
                flag_data, _ = self._zk_client.get(host_path)
                flag = flag_data.decode('utf-8')
                if flag == "new":
                    self.logger.info("The node {} is a new node, write the replica".format(self._host_node_name))
                    postgresql.demote_for_start(self._pgdata_dir, self._cluster_leader_ip, self._meta_port, self._user_name)
                    ...
                    if not postgresql.is_standby(self._pgdata_dir):
                        try:
                            ...
                            pass
                        postgresql.do_demote(self._pgdata_dir, self._cluster_leader_ip, self._meta_port, self._user_name, self._pod_ip, self._meta_port)
                        self._zk_client.create(membership_path)
                    else:
                        postgresql.change_following_leader(self._pgdata_dir, self._cluster_leader_ip, self._meta_port, self._user_name)
        else:
            postgresql.demote_for_start(self._pgdata_dir, self._cluster_leader_ip, self._meta_port, self._user_name)
    ...
    def find_node_cluster(self):
        ...
        if self._zk_client.exists(self._root_path + "/ready"):
            ...
            if not found_cluster:
                ...
                if self._is_cn:
                    ...
                    self._zk_client.create("{}/{}".format(self._dn_supplement_path, self._host_node_name), ephemeral=True)
                postgresql.clean_for_supplement(self._pod_ip, self._meta_port, self._user_name)
                self.logger.info("The node {} is not in any cluster, add it to the supplement cluster".format(self._host_node_name))
        if self._is_cn:
            if not found_cluster:
        ...
        while not found_cluster:
            ...
            if len(visited_cluster) > 0 and len(visited_cluster) == dn_cluster_num:
                ...
                if not self._zk_client.exists(self._dn_supplement_path + "/" + self._host_node_name):
                    self._zk_client.create("{}/{}".format(self._dn_supplement_path, self._host_node_name), ephemeral=True)
                    postgresql.clean_for_supplement(self._pod_ip, self._meta_port, self._user_name)
                    self.logger.info("The node {} is not in any cluster, add it to the supplement cluster".format(self._host_node_name))
            time.sleep(10)
    ...
    def watch_leader_and_candidates(self):
        leader_path = "{}/{}".format(self._leader_path, self._cluster_name)
        candidates_path = "{}/{}/candidates".format(self._cluster_path, self._cluster_name)
    ...
    def handle_leader_delete_event(self):
        candidate_path = "{}/{}/candidates".format(self._cluster_path, self._cluster_name)
        if postgresql.is_standby(self._pgdata_dir) and self.is_in_membership():
            candidate_node_path = "{}/{}:{}".format(candidate_path, self._pod_ip, self._meta_port)
            if postgresql.stop_replication(self._pod_ip, self._meta_port, self._user_name):
    ...
    def handle_leader_create_event(self):
        ...
        try:
            ...
            pass
        try:
            self._zk_client.delete("{}/{}/replicas/{}".format(self._cluster_path, self._cluster_name, ip_port))
        except Exception:
            pass
        self._cluster_leader_ip = leader_ip
        self.logger.info("The leader of the cluster {} is {}".format(self._cluster_name, leader_ip))
        ...
        if leader_ip == self._pod_ip:
            ...
            if postgresql.is_standby(self._pgdata_dir):
                postgresql.do_promote(self._pgdata_dir, self._pod_ip, self._meta_port, self._user_name)
            
            ret = False
            while not ret:
                self.logger.info("--update background service--")
                ret = postgresql.update_start_background_service(self._cluster_leader_ip, self._meta_port, self._user_name)
                if ret:
                    break
                time.sleep(1)
            ...
            while not ret:
                ...
                ret = postgresql.update_node_table(cn_leader_ip, self._meta_port, self._user_name, self._cluster_id, self._cluster_leader_ip, self._meta_port)
                postgresql.reload_foreign_server_cache(cn_leader_ip, self._meta_port, self._user_name)
                if ret:
                    break
                cn_leader_ip = self.get_cn_leader()
                time.sleep(1)
            ...
            if postgresql.is_standby(self._pgdata_dir):
                ...
                try:
                    self._zk_client.delete("{}/{}/membership/{}".format(self._cluster_path, self._cluster_name, self._host_node_name))
                except NoNodeError:
                    pass
                postgresql.do_demote(self._pgdata_dir,self._cluster_leader_ip, self._meta_port, self._user_name, self._pod_ip, self._meta_port)
                self._zk_client.create("{}/{}/membership/{}".format(self._cluster_path, self._cluster_name, self._host_node_name))
        self._is_changing = False
    
    def handle_candidate_change_event(self, candidates):
        leader_path = "{}/{}".format(self._leader_path, self._cluster_name)
        candidate_path = "{}/{}/candidates".format(self._cluster_path, self._cluster_name)
        self.logger.info("The candidate of the cluster {} is {}".format(self._cluster_name, len(candidates)))
        if len(candidates) >= self._replica_server_num:
            ...
            if new_host == self._pod_ip:
                try:
                    self._zk_client.create(leader_path, value=str.encode(new_host_port), ephemeral=True)
                    self._zk_client.set("{}/{}/lastLeader".format(self._cluster_path, self._cluster_name), value=str.encode(self._host_node_name))
                    try:
                        self._zk_client.delete("{}/{}/replicas/{}".format(self._cluster_path, self._cluster_name, new_host_port))
                    except NoNodeError:
                        pass
                except NodeExistsError:
                    pass
    ...
    def watch_conn(self):
        def conn_state_listener(state):
            if state == KazooState.LOST:
                self.logger.critical("The connection to the zookeeper is lost, please check the zookeeper status")
    ...
    def build_cluster(self):
        ...
        try:
            self._zk_client.create("{}/{}/hostNodes/{}".format(self._cluster_path, self._cluster_name, self._host_node_name))
        ...
        for i in range(live_cn_num):
            if self._cn_list[i] == self._host_node_name:
                ...
                try:
                    self._zk_client.create("{}/{}/hostNodes/{}".format(self._cluster_path, self._cluster_name, self._cn_list[i]))
                except Exception as ex:
                    self.logger.error("Failed to build the cluster: {}".format(ex))
                    raise ex
                idx += 1
        ...
        for dn_idx in range(cluster_num):
            ...
            for i in range(self._replica_server_num + 1):
                try:
                    self._zk_client.create("{}/hostNodes/{}".format(dn_cluster_path, self._dn_list[node_idx]))
                    node_idx += 1
                except Exception as ex:
                    self.logger.error("Failed to build the cluster: {}".format(ex))
                    raise ex
    ...
    def init_filesystem(self):
        ...
        while len(leader_infos) < len(self._cluster_names):
            for cluster_name in self._cluster_names:
                ...
                if cluster_name in leaders and len(replica_nodes) == self._replica_server_num:
                    cluster_leader_path = "{}/{}".format(self._leader_path, cluster_name)
    ...
    def build_all_membership(self):
        ...
        for cluster_name in self._cluster_names:
            host_nodes = self._zk_client.get_children("{}/{}/hostNodes".format(self._cluster_path, cluster_name))
            for host_node in host_nodes:
                membership_path = "{}/{}/membership/{}".format(self._cluster_path, cluster_name, host_node)
                try:
                    self._zk_client.create(membership_path)
                except NodeExistsError:
                    pass
    ...
    def is_in_membership(self):
        node_membership_path = "{}/{}/membership/{}".format(self._cluster_path, self._cluster_name, self._host_node_name)
    ...
    def start_watch_replica_thread(self):
        watch_replica_thread = threading.Thread(target=CuckooCM.handle_replica_change, args=(self,))
        watch_replica_thread.start()
        watch_replica_thread.join()
    
    def start_watch_replica_need_supplement_thread(self):
        watch_replica_thread = threading.Thread(target=CuckooCM.handle_replica_change, args=(self,))
        handle_replica_thread = threading.Thread(target=CuckooCM.handle_need_supplement, args=(self,))
        watch_replica_thread.start()
    ...
    def check_all_nodes_status(self, retry_num):
        isCheckStatus = True
        host_nodes_path = "{}/{}/hostNodes".format(self._cluster_path, self._cluster_name)
        host_membership_path = "{}/{}/membership".format(self._cluster_path, self._cluster_name)
        ...
        if isCheckStatus and retry_num == 0:
            for name in host_nodes:
                ...
                if self._zk_client.exists(node_path):
                    continue
                if self._lost_node_time[name] >= self._wait_replica_time:
                    self.logger.info("The node {} is lost, please check the status".format(name))
                    try:
                        self._zk_client.create("{}/{}-0".format(self._need_supplement_path, self._cluster_name))
                    except NodeExistsError:
                        try:
                            self._zk_client.create("{}/{}-1".format(self._need_supplement_path, self._cluster_name))
                        except NodeExistsError:
                            pass
        ...
        while self._thread_running:
            ...
            if need_supplement_num > 0:
                ...
                for child in children:
                    ...
                    if not self._zk_client.exists(self._need_supplement_path + "/" + child):
                        continue
                    node_name = ""
                    if child == 'cn-0' or child == 'cn-1':
                        while True:
                            sup_node_list = self._zk_client.get_children(self._cn_supplement_path)
                            if len(sup_node_list) > 0:
                               node_name = sup_node_list[0]
                               self._zk_client.delete(self._cn_supplement_path + "/" + node_name)
                               break
                            time.sleep(10)
                    else:
                        while True:
                            sup_node_list = self._zk_client.get_children(self._dn_supplement_path)
                            if len(sup_node_list) > 0:
                                node_name = sup_node_list[0]
                                self._zk_client.delete(self._dn_supplement_path + "/" + node_name)
                                break
                            time.sleep(10)
                    try:
                        self._zk_client.create("{}/hostNodes/{}".format(cluster_path, node_name), value=str.encode("new"))
                        self.logger.info("The node {} is added to the cluster {}".format(node_name, child[0:-2]))
                    except NodeExistsError:
                        pass
                    except Exception as ex:
                        pass
    ...
    def set_store_cluster_ready(self):
        try:
            self._zk_client.set(self._store_node_path + "/storeNodeStatus", value=str.encode('1'))
        except Exception as e:
            pass
